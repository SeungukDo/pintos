...
/* Defalut nice value for all threads */
#define NICE_DEFAULT 0

/* Default recent_cpu value for all threads */
#define RECENT_CPU_DEFAULT 0

/* Default load average value for all threads */
...
int load_avg;                 /* average # of excecutable process for recent 1 minutes */
...
/* compute priority using recent cpu and nice */
...
{
  //load_avg = add_fp(mult_fp(div_mixed(59 << 14, 60), load_avg), mult_mixed(div_mixed((1 << 14), 60), list_size(&ready_list)));
  int ready_threads;
  ...
  load_avg = mult_fp((int_to_fp(59) / 60), load_avg) + (int_to_fp(1) / 60) * ready_threads;
  ASSERT(load_avg >= 0);
}

/* increase recent_cpu 1 */
...
int thread_get_load_avg(void)
{
  enum intr_level old_level;
  old_level = intr_disable();
  int load_avg_round = fp_to_int_round(mult_mixed(load_avg, 100));
  //printf("load_avg: %x\n", load_avg);
  //printf("load_avg_multi: %x\n", mult_mixed(load_avg, 100));
  //printf("load_avg_round: %d\n", load_avg_round);
  intr_set_level(old_level);
  return load_avg_round;
}
...
/* Change current thread into THREAD_BLOCKED 
